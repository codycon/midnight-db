    ...
    try {
      ...
      if (!rules.length) {
        return interaction.editReply({
          content:
            "No automod rules configured yet. Use `/automod-setup` to create one.",
        });
      }
      ...
      for (const rule of rules.slice(0, 25)) {
        ...
        if (rule.violation_count > 1)
          lines.push(`**Violations:** ${rule.violation_count}`);
        if (rule.mute_duration)
          lines.push(
            `**Mute Duration:** ${formatDuration(rule.mute_duration)}`,
          );
      ...
      await interaction.editReply({
        content: "Failed to retrieve the rules list. Please try again.",
      });
    }
  },
};
