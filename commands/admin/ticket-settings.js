...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(s => s
            .setName('view')
            .setDescription('Show current ticket settings'))
        .addSubcommand(s => s
            .setName('log-open')
        ...
        .addSubcommand(s => s
            .setName('log-close')
            .setDescription('Set the channel where closed tickets and transcripts are logged')
            .addChannelOption(o => o.setName('channel').setDescription('Log channel').setRequired(true)))
        .addSubcommand(s => s
            .setName('category')
            .setDescription('Set the default Discord category for new ticket channels')
    ...
    async execute(interaction) {
        ...
        try {
            switch (sub) {
                case 'view':               return await this._view(interaction);
                case 'log-open':           return await this._logOpen(interaction);
                case 'log-close':          return await this._logClose(interaction);
                case 'category':           return await this._setCategory(interaction);
                case 'staff-role-add':     return await this._addStaffRole(interaction);
                case 'staff-role-remove':  return await this._removeStaffRole(interaction);
            }
        } catch (err) {
    ...
    async _view(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                {
                    name:  'Open Log',
                    value: settings.log_open  ? `<#${settings.log_open}>`  : 'Not configured',
                    inline: true,
                },
                {
                    name:  'Close Log + Transcripts',
                    value: settings.log_close ? `<#${settings.log_close}>` : 'Not configured',
                    inline: true,
                },
                {
                    name:  'Default Category',
                    value: settings.default_cat ? `<#${settings.default_cat}>` : 'None (channels created at root level)',
                    inline: true,
                },
                {
    ...
    async _logClose(interaction) {
        ...
        return interaction.editReply({ content: `Close log (and transcripts) set to ${channel}.` });
    },
    ...
    async _setCategory(interaction) {
        ...
        if (category.type !== ChannelType.GuildCategory) {
            return interaction.editReply({ content: 'Please select a category channel.' });
        }

        tdb.upsertSettings(interaction.guild.id, { default_cat: category.id });
        return interaction.editReply({ content: `Default category set to **${category.name}**.` });
    },
    ...
    async _removeStaffRole(interaction) {
        ...
        if (!result.changes) {
            return interaction.editReply({ content: `${role} is not in the staff roles list.` });
        }
        return interaction.editReply({ content: `${role} removed from staff roles.` });
    },
};
