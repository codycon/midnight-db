...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((o) =>
      o
        .setName("threshold")
        .setDescription("Trigger threshold (meaning depends on rule type)")
        .setRequired(false),
    )
    .addIntegerOption((o) =>
      o
        .setName("violations")
        .setDescription(
          "Violations required before action triggers (auto_mute / auto_ban only)",
        )
        .setRequired(false)
        .setMinValue(1)
        .setMaxValue(10),
    ...
    .addChannelOption((o) =>
      o
        .setName("log-channel")
        .setDescription("Override the default log channel for this rule")
        .setRequired(false),
    )
    .addStringOption((o) =>
      o
        .setName("custom-message")
        .setDescription("Custom text for warn messages")
        .setRequired(false),
    ),
  ...
  async execute(interaction) {
    ...
    const threshold =
      ...
      DEFAULT_THRESHOLDS[ruleType] ??
      null;
    const violations =
      interaction.options.getInteger("violations") ??
      DEFAULT_VIOLATIONS[action] ??
      1;
    ...
    try {
      ...
      await interaction.editReply({
        content: "Failed to create the automod rule. Please try again.",
      });
    }
  },
};
