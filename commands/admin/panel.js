...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(s => s
            ...
            .addStringOption(o => o.setName('name').setDescription('Internal name for this panel').setRequired(true))
            .addStringOption(o => o
                .setName('type')
            ...
            .addStringOption(o => o
                .setName('style')
                .setDescription('Default ticket style (can be overridden per option)')
                .addChoices(
        ...
        .addSubcommand(s => s
            ...
            .addStringOption(o => o.setName('color').setDescription('Hex colour, e.g. 5865F2'))
            .addStringOption(o => o.setName('footer').setDescription('Footer text'))
            .addStringOption(o => o.setName('image').setDescription('Full image URL'))
        ...
        .addSubcommand(s => s
            ...
            .addIntegerOption(o => o.setName('embed-id').setDescription('Embed ID (from /panel list)').setRequired(true)))
        ...
        .addSubcommand(s => s
            ...
            .addIntegerOption(o => o.setName('panel').setDescription('Panel ID').setRequired(true))
            .addStringOption(o => o.setName('label').setDescription('Button or option label').setRequired(true))
            .addStringOption(o => o.setName('emoji').setDescription('Emoji to display on the button or option'))
            .addStringOption(o => o.setName('description').setDescription('Short description shown in select menus'))
            .addStringOption(o => o
                .setName('button-style')
                .setDescription('Button colour (only applies to button-type panels)')
                .addChoices(
            ...
            .addStringOption(o => o
                ...
                .setDescription('Override the panel\'s default ticket style for this option only')
                .addChoices(
            ...
            .addStringOption(o => o
                ...
                .setDescription('Channel name format. Tokens: {username} {number} {tag}')))
        .addSubcommand(s => s
            .setName('option-config')
            .setDescription('Set roles and category for a panel option')
            .addIntegerOption(o => o.setName('option-id').setDescription('Option ID (from /panel list)').setRequired(true))
            .addChannelOption(o => o.setName('category').setDescription('Discord category for tickets created by this option'))
            .addStringOption(o => o.setName('support-roles').setDescription('Comma-separated role IDs that can see these tickets'))
            .addStringOption(o => o.setName('required-roles').setDescription('Comma-separated role IDs required to open this ticket type')))
        .addSubcommand(s => s
        ...
        .addSubcommand(s => s
            ...
            .setDescription('Add a modal question to a panel option (max 5 per option)')
            .addIntegerOption(o => o.setName('option-id').setDescription('Option ID').setRequired(true))
            .addStringOption(o => o.setName('label').setDescription('Question text shown to the user').setRequired(true))
            .addStringOption(o => o
                .setName('style')
            ...
            .addStringOption(o => o.setName('placeholder').setDescription('Placeholder text inside the field'))
            .addBooleanOption(o => o.setName('required').setDescription('Whether the user must answer this question'))
            .addIntegerOption(o => o.setName('min-length').setDescription('Minimum answer length'))
            .addIntegerOption(o => o.setName('max-length').setDescription('Maximum answer length')))
        .addSubcommand(s => s
            .setName('question-remove')
            .setDescription('Remove a question from an option')
            .addIntegerOption(o => o.setName('question-id').setDescription('Question ID (from /panel list)').setRequired(true))),
    ...
    async execute(interaction) {
        ...
        try {
            switch (sub) {
                ...
                case 'post':            return await this._post(interaction);
                case 'list':            return await this._list(interaction);
                case 'delete':          return await this._delete(interaction);
                case 'embed-add':       return await this._embedAdd(interaction);
                case 'embed-remove':    return await this._embedRemove(interaction);
                case 'option-add':      return await this._optionAdd(interaction);
                case 'option-config':   return await this._optionConfig(interaction);
                case 'option-remove':   return await this._optionRemove(interaction);
                case 'question-add':    return await this._questionAdd(interaction);
                case 'question-remove': return await this._questionRemove(interaction);
            }
        } catch (err) {
    ...
    async _post(interaction) {
        ...
        if (!panel.options?.length) {
            return interaction.editReply({ content: 'Add at least one option before posting the panel.' });
        }
    ...
    async _list(interaction) {
        ...
        if (!panels.length) {
            return interaction.editReply({ content: 'No panels yet. Use `/panel create` to get started.' });
        }
        ...
        for (const p of panels.slice(0, 10)) {
            ...
            const embedLines  = full.embeds.map(e  => `  [${e.id}] ${e.title ?? '(no title)'}`);
            const optionLines = full.options.map(o =>
                `  [${o.id}] ${o.label}` + (o.questions.length ? ` (${o.questions.length} question(s))` : '')
            );
    ...
    async _delete(interaction) {
        ...
        return interaction.editReply({ content: `Panel **"${panel.name}"** deleted.` });
    },
    ...
    async _embedAdd(interaction) {
        ...
        if (panel.embeds.length >= 10) {
            return interaction.editReply({ content: 'Panels can have at most 10 embeds (Discord limit).' });
        }
    ...
    async _optionAdd(interaction) {
        ...
        if (panel.options.length >= 25) {
            return interaction.editReply({ content: 'Panels can have at most 25 options.' });
        }
    ...
    async _optionConfig(interaction) {
        ...
        if (supportRaw)  updates.support_roles  = supportRaw.split(/[\s,]+/).filter(Boolean).join(',');
        if (requiredRaw) updates.required_roles = requiredRaw.split(/[\s,]+/).filter(Boolean).join(',');

        if (!Object.keys(updates).length) {
            return interaction.editReply({ content: 'Nothing to update. Provide at least one option.' });
        }
        ...
        return interaction.editReply({ content: `Option **"${option.label}"** updated.\n${lines.join('\n')}` });
    },
    ...
    async _optionRemove(interaction) {
        ...
        return interaction.editReply({ content: `Option **"${option.label}"** removed.` });
    },
    ...
    async _questionAdd(interaction) {
        ...
        if (option.questions.length >= 5) {
            return interaction.editReply({ content: 'Options can have at most 5 questions (Discord modal limit).' });
        }
    ...
    async _questionRemove(interaction) {
        ...
        return interaction.editReply({ content: `Question #${questionId} removed.` });
    },
};
