...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s.setName("list").setDescription("List all configured domain filters"),
    ),
  ...
  async execute(interaction) {
    ...
    try {
      switch (sub) {
        case "allow":
          return await this._allow(interaction);
        case "block":
          return await this._block(interaction);
        case "list":
      ...
      await interaction.editReply({
        content: "Failed to manage link filters. Please try again.",
      });
    }
  },
  ...
  async _allow(interaction) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({
        name: "Note",
        value:
          'To use the allowlist, create a "Links" rule with threshold set to 1 (allowlist mode).',
      });
  ...
  async _block(interaction) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields({
        name: "Note",
        value:
          'Ensure a "Links" automod rule is enabled for this to take effect.',
      });
  ...
  async _list(interaction) {
    ...
    if (!allowed.length && !blocked.length) {
      return interaction.editReply({
        content:
          "No domain filters configured. Use `/automod-links allow` or `/automod-links block` to add some.",
      });
    }
    ...
    if (allowed.length)
      embed.addFields({
        name: `Allowed Domains (${allowed.length})`,
        value: truncate(allowed),
      });
    if (blocked.length)
      embed.addFields({
        name: `Blocked Domains (${blocked.length})`,
        value: truncate(blocked),
      });
