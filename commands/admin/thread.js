...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('thread')
        .setDescription('Create a private staff-only discussion thread inside the current ticket')
        .addStringOption(o => o
            .setName('topic')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            return interaction.editReply({ content: `Staff thread created: ${thread}` });
        } catch (err) {
            console.error('[THREAD]', err);
            return interaction.editReply({ content: `Failed to create staff thread: ${err.message}` });
        }
    },
};
