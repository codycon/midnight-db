...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            switch (sub) {
                ...
                case 'log-channel':    return await this._setLogChannel(interaction);
                case 'ignore-role':    return await this._addIgnoredRole(interaction);
                case 'ignore-channel': return await this._addIgnoredChannel(interaction);
            }
        } catch (err) {
            console.error('[AUTOMOD] Error in automod-settings:', err);
            await interaction.editReply({ content: 'Failed to update settings. Please try again.' });
        }
    },
    ...
    async _view(interaction) {
        ...
        if (!settings) {
            embed.setDescription('No settings configured. Use the other subcommands to get started.');
        } else {
            if (settings.default_log_channel) {
                embed.addFields({ name: 'Default Log Channel', value: `<#${settings.default_log_channel}>` });
            }
            ...
            embed.addFields({
                name: 'Always Ignored',
                value: 'Server owner and all administrators are always exempt from automod.',
            });
        }
    ...
    async _setLogChannel(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Note', value: 'Individual rules can override this with their own log channel.' });
    ...
    async _addIgnoredRole(interaction) {
        ...
        if (ignoredRoles.includes(role.id)) {
            return interaction.editReply({ content: `${role} is already in the ignore list.` });
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Note', value: 'Server owner and administrators are always exempt regardless of this setting.' });
    ...
    async _addIgnoredChannel(interaction) {
        ...
        if (ignoredChannels.includes(channel.id)) {
            return interaction.editReply({ content: `${channel} is already in the ignore list.` });
        }
