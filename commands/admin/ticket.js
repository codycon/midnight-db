...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(s => s
            .setName('delete')
            .setDescription('Close, generate a transcript, then delete the ticket channel')
            .addStringOption(o => o.setName('reason').setDescription('Reason for deletion')))
        .addSubcommand(s => s
            .setName('add')
        ...
        .addSubcommand(s => s
            .setName('transcript')
            .setDescription('Generate and download a transcript of the current ticket'))
        .addSubcommand(s => s
            .setName('history')
            .setDescription('Search ticket history')
            .addStringOption(o => o.setName('query').setDescription('Search by ticket name, user ID, channel ID, or ticket number'))
    ...
    async execute(interaction) {
        ...
        switch (sub) {
            case 'close':  return this._close(interaction, ticket, isStaff);
            case 'delete': return this._delete(interaction, ticket, isStaff);
            case 'add':    return this._add(interaction, ticket, isStaff);
            case 'remove': return this._remove(interaction, ticket, isStaff);
            case 'rename': return this._rename(interaction, ticket, isStaff);
        }
    },
    ...
    async _delete(interaction, ticket, isStaff) {
        ...
        const reason  = interaction.options.getString('reason') ?? 'Deleted by staff';
        ...
        await manager.logClose(interaction.guild, { ...ticket, status: 'deleted' }, buf, interaction.user, settings);

        await interaction.editReply({ content: 'Ticket closed. Deleting channel in a moment...' });
    ...
    async _add(interaction, ticket, isStaff) {
        ...
        try {
            ...
            await channel.send({ content: `${target} has been added to the ticket.` });
            return interaction.reply({ content: `${target} added.`, ephemeral: true });
        } catch (err) {
    ...
    async _remove(interaction, ticket, isStaff) {
        ...
        if (target.id === ticket.user_id) {
            return interaction.reply({ content: 'The ticket owner cannot be removed.', ephemeral: true });
        }
        ...
        try {
            ...
            await channel.send({ content: `${target} has been removed from the ticket.` });
            return interaction.reply({ content: `${target} removed.`, ephemeral: true });
        } catch (err) {
    ...
    async _rename(interaction, ticket, isStaff) {
        ...
        try {
            ...
            return interaction.reply({ content: `Rename failed: ${err.message}`, ephemeral: true });
        }
    },
    ...
    async _transcript(interaction) {
        ...
        if (!isStaff && interaction.user.id !== ticket.user_id) {
            return interaction.reply({ content: 'Only staff or the ticket owner can generate a transcript.', ephemeral: true });
        }

        await interaction.deferReply({ ephemeral: true });

        try {
            const buf  = await transcript.fromChannel(interaction.channel, ticket, interaction.guild);
            ...
            return interaction.editReply({ content: `Failed to generate transcript: ${err.message}` });
        }
    },
    ...
    async _history(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`${tickets.length} result(s)${user ? ` for ${user.tag}` : ''}`)
            .setTimestamp();
