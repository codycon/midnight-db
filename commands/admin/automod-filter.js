...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(s => s
            ...
            .addStringOption(o => o
                .setName('filter-type')
                .setDescription('Whether the filter is an allowlist or blocklist for this rule')
                .setRequired(true)
                .addChoices(
                    { name: 'Affected — rule only applies to these', value: 'affected' },
                    { name: 'Ignored  — rule never applies to these', value: 'ignored'  }
                ))
            .addStringOption(o => o
                .setName('target-type')
    ...
    async execute(interaction) {
        ...
        try {
            switch (sub) {
                case 'add':  return await this._add(interaction);
                case 'list': return await this._list(interaction);
            }
        } catch (err) {
            console.error('[AUTOMOD] Error in automod-filter:', err);
            await interaction.editReply({ content: 'Failed to manage filters. Please try again.' });
        }
    },
    ...
    async _add(interaction) {
        ...
        const targetId = interaction.options.getString('target-id').replace(/[<@&#>]/g, '').trim();

        const rule = db.getRuleById(ruleId);
        if (!rule || rule.guild_id !== interaction.guild.id) {
            return interaction.editReply({ content: 'Rule not found. Use `/automod-list` to see valid rule IDs.' });
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: 'Rule',        value: `[${rule.id}] ${formatRuleName(rule.rule_type)}` },
                { name: 'Filter Type', value: filterType.charAt(0).toUpperCase() + filterType.slice(1), inline: true },
                { name: 'Target',      value: targetType === 'role' ? `<@&${targetId}>` : `<#${targetId}>`, inline: true }
            );
    ...
    async _list(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`**${formatRuleName(rule.rule_type)}** — ${filters.length} filter(s)`);

        if (affectedRoles.length)    embed.addFields({ name: 'Affected Roles',    value: affectedRoles.map(f    => `<@&${f.target_id}>`).join(', ') });
        if (affectedChannels.length) embed.addFields({ name: 'Affected Channels', value: affectedChannels.map(f => `<#${f.target_id}>`).join(', ')  });
        if (ignoredRoles.length)     embed.addFields({ name: 'Ignored Roles',     value: ignoredRoles.map(f     => `<@&${f.target_id}>`).join(', ') });
        if (ignoredChannels.length)  embed.addFields({ name: 'Ignored Channels',  value: ignoredChannels.map(f  => `<#${f.target_id}>`).join(', ')  });
