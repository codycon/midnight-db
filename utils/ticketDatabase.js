...
class TicketDatabase {
  ...
  getSettings(guildId) {
    return (
      this.db
        .prepare("SELECT * FROM ticket_settings WHERE guild_id = ?")
        .get(guildId) ?? {}
    );
  }
  ...
  nextTicketNumber(guildId) {
    const row = this.db
      .prepare("SELECT ticket_counter FROM ticket_settings WHERE guild_id = ?")
      .get(guildId);
  ...
  getPanel(panelId) {
    const panel = this.db
      .prepare("SELECT * FROM ticket_panels WHERE id = ?")
      .get(panelId);
    if (!panel) return null;

    panel.embeds = this.db
      .prepare(
        "SELECT * FROM ticket_panel_embeds   WHERE panel_id  = ? ORDER BY position",
      )
      .all(panelId);
    panel.options = this.db
      .prepare(
        "SELECT * FROM ticket_panel_options  WHERE panel_id  = ? ORDER BY position",
      )
      .all(panelId);

    for (const option of panel.options) {
      option.questions = this.db
        .prepare(
          "SELECT * FROM ticket_option_questions WHERE option_id = ? ORDER BY position",
        )
        .all(option.id);
  ...
  getPanels(guildId) {
    return this.db
      .prepare("SELECT * FROM ticket_panels WHERE guild_id = ? ORDER BY id")
      .all(guildId);
  }
  ...
  updatePanel(panelId, fields) {
    const sets = Object.keys(fields)
      ...
      .join(", ");
    this.db
      .prepare(`UPDATE ticket_panels SET ${sets} WHERE id = ?`)
      .run(...Object.values(fields), panelId);
  }

  deletePanel(panelId) {
    return this.db
      .prepare("DELETE FROM ticket_panels WHERE id = ?")
      .run(panelId);
  }
  ...
  addEmbed(panelId, data) {
    const pos = this.db
      .prepare(
        "SELECT COUNT(*) AS c FROM ticket_panel_embeds WHERE panel_id = ?",
      )
      .get(panelId).c;
    return this.db
      .prepare(
  ...
  deleteEmbed(embedId) {
    return this.db
      .prepare("DELETE FROM ticket_panel_embeds WHERE id = ?")
      .run(embedId);
  }
  ...
  addOption(panelId, data) {
    const pos = this.db
      .prepare(
        "SELECT COUNT(*) AS c FROM ticket_panel_options WHERE panel_id = ?",
      )
      .get(panelId).c;
    return this.db
      .prepare(
  ...
  getOption(optionId) {
    const option = this.db
      .prepare("SELECT * FROM ticket_panel_options WHERE id = ?")
      .get(optionId);
  ...
  updateOption(optionId, data) {
    ...
    for (const col of ALLOWED_COLUMNS) {
      ...
      fields[col] =
        col === "support_roles" || col === "required_roles"
    ...
    const sets = Object.keys(fields)
      ...
      .join(", ");
    this.db
      .prepare(`UPDATE ticket_panel_options SET ${sets} WHERE id = ?`)
      .run(...Object.values(fields), optionId);
  }

  deleteOption(optionId) {
    return this.db
      .prepare("DELETE FROM ticket_panel_options WHERE id = ?")
      .run(optionId);
  }
  ...
  addQuestion(optionId, data) {
    const pos = this.db
      .prepare(
        "SELECT COUNT(*) AS c FROM ticket_option_questions WHERE option_id = ?",
      )
      .get(optionId).c;
    return this.db
      .prepare(
  ...
  deleteQuestion(questionId) {
    return this.db
      .prepare("DELETE FROM ticket_option_questions WHERE id = ?")
      .run(questionId);
  }
  ...
  getTicketByChannel(channelId) {
    const row = this.db
      .prepare("SELECT * FROM tickets WHERE channel_id = ? OR thread_id = ?")
  ...
  updateTicket(ticketId, fields) {
    const sets = Object.keys(fields)
      ...
      .join(", ");
    this.db
      .prepare(`UPDATE tickets SET ${sets} WHERE id = ?`)
      .run(...Object.values(fields), ticketId);
  }
