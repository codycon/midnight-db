...
class TicketManager {
  ...
  async createTicket(interaction, panel, option, answers = {}) {
    ...
    if (option?.required_roles?.length) {
      const hasRole = option.required_roles.some((id) =>
        member.roles.cache.has(id),
      );
      if (!hasRole) {
        return interaction.reply({
          content:
            "You do not have the required role to open this ticket type.",
          ephemeral: true,
        });
      }
    }
    ...
    // --- Deduplication check ---
    const existing = tdb.getOpenTicketByUser(
      guild.id,
      member.id,
      option?.id ?? null,
    );
    ...
    if (!channel) {
      return interaction.reply({
        content:
          "Failed to create a ticket channel. Please contact an administrator.",
        ephemeral: true,
      });
    }
  ...
  async _createThread(guild, panel, option, member, ticketName) {
    ...
    const parentChannel =
      (panel.channel_id && guild.channels.cache.get(panel.channel_id)) ??
    ...
    if (!parentChannel) {
      console.error(
        "[TICKET] No suitable parent channel found for thread ticket.",
      );
      return null;
    }
    ...
    try {
      ...
      for (const roleId of roleIds) {
        ...
        for (const [, m] of guild.members.cache.filter((m) =>
  ...
  async closeTicket(interaction, ticketId, confirm = false) {
    ...
    try {
      ...
      console.error(
        "[TICKET] Live transcript failed, falling back to DB:",
        err,
      );
    ...
    const closeEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `Closed by ${interaction.user}. A transcript has been sent to the log channel.`,
      )
      .setTimestamp();

    await channel.send({ embeds: [closeEmbed] });
    await this._logClose(
      guild,
      ticket,
      transcriptBuf,
      interaction.user,
    ...
    await interaction.editReply({
      content: "Ticket closed. Transcript has been sent to the log channel.",
    });
  }
  ...
  async _disableTicketButtons(channel) {
    try {
      ...
      for (const msg of messages.values()) {
        if (msg.author.id !== channel.client.user.id || !msg.components.length)
          continue;

        const updatedRows = msg.components.map((row) => {
          const buttons = row.components.map((btn) => {
            const isTicketButton =
              btn.customId?.startsWith("ticket_close") ||
  ...
  async _logClose(
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "Ticket",
          value: `#${ticket.ticket_number} — ${ticket.ticket_name}`,
          inline: true,
        },
        {
          name: "User",
          value: `<@${ticket.user_id}> (${ticket.user_id})`,
          inline: true,
        },
        {
          name: "Closed By",
          value: closedBy ? `${closedBy}` : "Channel deleted",
          inline: true,
        },
  ...
  async createStaffThread(interaction, ticketId) {
    ...
    try {
      ...
      for (const roleId of staffRoles) {
        for (const [id, m] of interaction.guild.members.cache.filter((m) =>
          m.roles.cache.has(roleId),
        )) {
          if (!added.has(id)) {
      ...
      await interaction.reply({
        content: "Failed to create staff thread.",
        ephemeral: true,
      });
    }
  }
