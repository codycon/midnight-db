    ...
    try {
      switch (rule.action) {
        ...
        case "auto_mute": {
          ...
          const muteCount = db.getViolationCount(
            message.guild.id,
            message.author.id,
            rule.rule_type,
          );
          ...
          if (muteCount >= muteTarget) {
            const ok = await this._muteUser(
              message.member,
              rule.mute_duration ?? 300,
            );
            actionTaken = ok
              ? `Auto-muted (${muteCount} violations)`
              : "Mute failed";
          } else {
        ...
        case "auto_ban": {
          db.addViolation(message.guild.id, message.author.id, rule.rule_type);
          const banCount = db.getViolationCount(
            message.guild.id,
            message.author.id,
            rule.rule_type,
          );
          ...
          if (banCount >= banTarget) {
            const ok = await this._banUser(
              message.member,
              `Automod: ${formatRuleName(rule.rule_type)}`,
            );
            actionTaken = ok
              ? `Auto-banned (${banCount} violations)`
              : "Ban failed";
          } else {
  ...
  async _logViolation(message, rule, actionTaken, settings) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `**Rule:** ${formatRuleName(rule.rule_type)}\n**Action:** ${actionTaken}`,
        )
        .addFields(
          {
            name: "User",
